...
def get_home_data(db: Session = Depends(get_db)):
    return {
        "projects": db.query(models.Project)
            .order_by(models.Project.display_order.asc(), models.Project.id.asc())
            .all(),

        "skills": db.query(models.Skill)
            .filter(models.Skill.is_active == True)
            .all(),

        "experiences": db.query(models.Experience)
            .order_by(models.Experience.display_order.asc(), models.Experience.id.asc())
            .all(),

        "education": db.query(models.Education)
            .order_by(models.Education.display_order.asc(), models.Education.id.asc())
            .all(),

        "research_papers": db.query(models.ResearchPaper)
            .all(),

        "certifications": db.query(models.Certification)
            .order_by(models.Certification.display_order.asc(), models.Certification.id.asc())
            .all(),

        "technical_events": db.query(models.TechnicalEvent)
            .order_by(models.TechnicalEvent.display_order.asc(), models.TechnicalEvent.id.asc())
            .all(),

        "achievements": db.query(models.Achievement)
            .order_by(models.Achievement.display_order.asc(), models.Achievement.id.asc())
            .all(),

        "social_links": db.query(models.SocialLink)
            .all(),

        "site_settings": {
            s.key: s.value
            for s in db.query(models.SiteSetting).all()
        }
    }
...
def get_projects(db: Session = Depends(get_db)):
    return db.query(models.Project).order_by(
        models.Project.display_order.asc(),
...
def get_experiences(db: Session = Depends(get_db)):
    return db.query(models.Experience).order_by(
        models.Experience.display_order.asc(),
...
def get_achievements(db: Session = Depends(get_db)):
    return db.query(models.Achievement).order_by(
        models.Achievement.display_order.asc(),
...
def get_education(db: Session = Depends(get_db)):
    return db.query(models.Education).order_by(
        models.Education.display_order.asc(),
...
def get_certifications(db: Session = Depends(get_db)):
    return db.query(models.Certification).order_by(
        models.Certification.display_order.asc(),
        models.Certification.id.asc()
    ).all()
...
def get_technical_events(db: Session = Depends(get_db)):
    return db.query(models.TechnicalEvent).order_by(
        models.TechnicalEvent.display_order.asc(),
        models.TechnicalEvent.id.asc()
    ).all()
